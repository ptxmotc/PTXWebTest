...
-  公車動態A1及A2相關狀態Status代碼意義補充說明：
     ...
     （1）	DutyStatus('0:正常','1: 開始','2:結束')：0通常是車機剛開機才會顯示的值；1是指一般車機行駛之狀態；2是車機行經公車路線之末端站位後，結束該趟次任務

     （2）	BusStatus('99: 非營運狀態')：99表示公車動態系統不會預估該車輛之預估到站時間，通常為該車輛不提供服務或車內場站調度才會切換的狀態，以避免影響動態正確性。

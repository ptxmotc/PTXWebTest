...
*   如何從路線班表(Schedule)資料，取得首末班車資訊呢?

    每條路線的預定時刻表(Timetables)，會有多筆班次(Trips)資料。 而每筆班次(Trip)下，會有多筆區分服務日(ServiceDay)的停靠時間資料(StopTimes)。若要取得路線中，不同方向(Direction)之首班車發車時刻，可從預定時刻表(Timetables)中，第一個Trip的停靠時間(StopTimes)，取得停靠路線站序為1(StopSequence=1)的離站時間(DepartureTime)，即為首班車發車時間；同理，若要取得路線中，不同方向(Direction)之末班車發車時刻，可從預定時刻表(Timetables)中，最後一個Trip的停靠時間(StopTimes)，取得停靠路線站序為1的離站時間(DepartureTime)，即為末班車發車時間。
*   起迄站中英文名稱：
